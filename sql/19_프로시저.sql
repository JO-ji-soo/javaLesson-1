...
BEGIN		--프로시저 시작(실행부)
  -- 프로시저 내부에는 주로 트랜잭션을 구성하는 야러개의 DML 명령문들을 작성하는 것이 일반적.
	SELECT name,age 
...
/*
	...
	( 매개변수이름 [IN 또는 OUT] 데이터 타입,.....)   IN : 입력매개변수, OUT : 출력매개변수(리턴값저장)
	[IS 또는 AS]
		프로시저 변수 선언
	BEGIN
		프로시저 작업 내용 SQL들....
...
CREATE OR REPLACE PROCEDURE max_custom(	  --자바의 메소드 인자와 같은 개념 / 리턴값 없고 리턴값까지도 인자로 선언
...
BEGIN
   SELECT max(quantity)					  --구매 수량 최대값
      INTO maxval 					      --조회 결과를 일반 변수에 저장
   FROM tbl_buy; 

   SELECT customid
      INTO cid							  --조회 결과를 일반 변수에 저장
   FROM tbl_buy
   WHERE quantity = maxval;

   SELECT name,age 
      INTO p_name , p_age   			  --출력 매개 변수에 저장
   FROM "TBL_CUSTOM" tc 
   WHERE CUSTOM_ID =cid;
